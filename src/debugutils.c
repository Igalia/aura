 ...
 */
void
my_debug_bin_to_dot_file (GstBin * bin, GstDebugGraphDetails details,
    const gchar * file_name)
{
  ...
  full_file_name = g_strdup_printf ("/tmp" G_DIR_SEPARATOR_S "%s.dot",
      file_name);
  ...
  if ((out = fopen (full_file_name, "wb"))) {
    ...
    fprintf(stderr, "Failed to open file '%s' for writing: %s", full_file_name,
        g_strerror (errno));
  }
